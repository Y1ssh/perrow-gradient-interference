...
GNCELoss:
    Routes the auxiliary signal through intermediate transformer layers
    using a contrastive (cosine-similarity, logsigmoid) embedding-matching
    loss, so the shared output projection (lm_head) receives ONLY the CE
    gradient. This capacity-separates the auxiliary from the output head.
    In the 124M Phase-B runs it does NOT recover the CE-only baseline:
    std G_nce is Delta +0.039 nats vs A (p ~ 0.010, distinguishable); the
    tuned (10,9) variant is Delta +0.018 (mean 4.0026), which is not
    statistically separable from A at n=3 (Welch p ~ 0.19) but whose point
    estimate is still ~1 sd above A (sd_A=0.014) — i.e. it does NOT
    demonstrate recovery. Earlier small-model (16M) exploratory numbers did
    not replicate at scale and are not used.
