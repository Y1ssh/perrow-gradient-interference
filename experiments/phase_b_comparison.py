...
The MAIN comparison of the paper. Measures whether routing the auxiliary
signal away from the shared output projection (G_nce) recovers the CE-only
baseline at 124M scale. (It does not: no capacity-separated auxiliary we
tried recovers A — neither standard nor tuned G_nce; see
model/auxiliary_losses.py for the per-variant statistics.) Note the per-row CE/MTP gradients are
directionally ALIGNED, not conflicting; the shared-head damage tracks
support/capacity competition, not gradient cancellation.

Variants:
    a       CE only (baseline)
    b       CE + shared multi-offset MTP (single tied head; damages CE)
    b_sg    CE + stop-grad MTP (lm_head detached for MTP — diagnostic)
    gnce    CE + NCE on intermediate layers (capacity-separated auxiliary)
