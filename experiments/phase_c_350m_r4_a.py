...
Trains GPT-2 Medium (354M) with CE only (variant A) for 87,000 steps.
...
RESULTS_DIR = os.path.join(
    ...
    'results', 'phase_c_350m_r4_a'
)
os.makedirs(RESULTS_DIR, exist_ok=True)

tokens_per_step = args.batch_size * args.seq_len
total_steps = args.steps
outfile = f'scale_350m_r4_a_seed{args.seed}_{args.steps}steps.json'
...
results = {'variant': '350m_r4_a', 'seed': args.seed, 'crashed': False}
try:
    results = train_350m()
except Exception as e:
    print(f"\n  *** CRASHED: {e}")
    import traceback; traceback.print_exc()
    results = {'variant': '350m_r4_a', 'seed': args.seed,
...
try:
    ...
    subprocess.run(['git', 'commit', '-m', f'Phase C 350M R4 variant A: {outfile}'],
...
print(f"\n{'=' * 60}")
print(f"  350M R≈{R_value:.2f} variant A (CE only): seed={args.seed}  steps={total_steps}")
