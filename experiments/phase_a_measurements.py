...
if a1_results.get('measurements') and a2_results.get('measurements'):
    ...
    print(f"\n  {'Val Loss':>10} | {'Muon Step':>10} | {'AdamW Step':>10} | "
          f"{'Muon disc*':>10} | {'AdamW disc*':>10}")
    # * disc = diagnostic ratio (per_row_0.3 / |global_cos|); ill-conditioned, not reported
    print(f"  {'-' * 62}")
    for mc in matched:
...
if a1_final:
    # Decision keys on the REPORTED quantity: per-row directional alignment.
    # (The old 'discrepancy'/PRDR ratio is ill-conditioned near global_cos~0
    #  and is diagnostic-only — see measurement/measure_interference.py.)
    align = a1_final['per_row_0.3']
    print(f"\n  DECISION:")
    if align > 0.5:
        print(f"  ✓ Strong per-row alignment at 124M (|cos|>0.3 in {align:.1%} of rows) — PROCEED")
    else:
        print(f"  ⚠ Per-row alignment lower than expected ({align:.1%})")
