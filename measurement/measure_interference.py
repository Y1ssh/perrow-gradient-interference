"""
Per-row gradient measurement at a shared output projection.

WHAT THIS MEASURES:
    When two losses (CE + MTP) share a weight matrix (the output projection
    lm_head), the aggregate (flattened) cosine between their gradients reads
    near 0. This is commonly read as "no conflict." We instead examine each
    ROW of the matrix (one per vocabulary token) and find the opposite of
    conflict: the per-row gradients are near-perfectly ALIGNED in direction
    (median per-row cosine ~ +1; only ~0.3% of rows have cosine < 0).

    The aggregate ~0 is therefore NOT cancellation of opposing gradients.
    It arises because the two losses put their gradient MAGNITUDE on
    different rows (disjoint support): where they overlap they agree, and
    the near-orthogonal aggregate reflects that they are largely active on
    different tokens. The operative axis is support, not direction.

    Implication: aggregate cosine-based conflict diagnostics (PCGrad,
    CAGrad, MGDA, and related) measure the wrong axis here — they see
    aggregate ~0 and infer conflict, when the per-row picture is alignment.

GEOMETRY:
    lm_head.weight has shape (vocab_size, n_embd); row i is the weight
    vector for token i. Per-row cosine compares the CE and aux gradients
    restricted to each row. A calibrated control (CE vs an unrelated L1
    loss) reads ~0.4% of rows above threshold, confirming the instrument
    is not trivially reporting alignment.
...
USAGE:
    ...
    print(f"Aggregate cos: {result['global_cos']:.3f}")
    print(f"Per-row |cos|>0.3: {result['per_row_fractions'][0.3]:.1%}")

    # Control (reads ~0% — calibrates the instrument)
    control = measure_interference(model, val_batch, loss_pair='ce_l1')

NOTE on 'discrepancy':
    The returned 'discrepancy' = per_row_0.3 / max(|global_cos|, 0.001) is a
    DIAGNOSTIC-ONLY ratio retained for backward compatibility with existing
    result JSONs. It is ill-conditioned when |global_cos| -> 0 and is NOT a
    reported quantity in the paper. Report the raw per-row distribution
    (median, %cos<0, %|cos|>0.3) and global_cos instead.
"""
...
def measure_interference(model, batch, loss_pair='ce_mtp',
    ...
    Returns: dict with:
        ...
        'discrepancy':       float   — DIAGNOSTIC ONLY (ill-conditioned; not reported). See module note.
